               ...
               int k, neighbor_t *results)
{
    ...
    /* ------------------------------------------------------------------
     * Bbox repair pass: scan ALL remaining clusters where
     * bbox_lower_bound <= worst candidate distance.
     * Guarantees 100% recall without full scan.
     * ------------------------------------------------------------------ */
    if (use_bbox && cand_size >= REFINE_K) {
        uint8_t scanned[IVF_NUM_CLUSTERS];
        memset(scanned, 0, sizeof(scanned));
        for (int p = 0; p < nprobe; p++)
            scanned[centroid_order[p]] = 1;
        ...
        for (int c = 0; c < num_clusters; c++) {
            ...
            int64_t lb = bbox_lower_bound(query_i16, index->bbox_min,
                                           index->bbox_max, c);
            if (lb > worst_i16) continue;

            /* This cluster may contain a better neighbor — scan it */
            if (use_quant) {
                scan_cluster_scalar(index, query_i16, start, end,
                                     candidates, &cand_size,
                                     &worst_cand_idx, &worst_i16);
